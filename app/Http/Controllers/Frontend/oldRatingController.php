...
class RatingController extends Controller
{
    public function suggestionAdd(Request $request)
    {
       $validator=  Validator::make($request->all(),[
        ...
        if($validator->fails()){
            return Helper::errorMessage($validator->errors());
        }else{
            $data=[];
            ...
            $message= 'Suggestion Send successfully';
    ...
    public function addTeacherRate(Request $request)
    {
        ...
        if($validator->fails()){
            ...
            if($checkTeacherRating){
                ...
                $message= 'Teacher Rating Successful';
                return Helper::successMessage($message,$data);
    ...
    public function AddTeacherLikeDislike(Request $request)
    {
        ...
        if($validator->fails()){
            ...
            Helper::Notification($userData->id,$replyData->user_id,$notiftype,$replyData->teacher_id,2,$Rnotiftype,$userData->systemNickname.' '.$typeName.' your review',$replyId);
    ...
    public function addTeacherReview(Request $request )
    {
        $validator= Validator::make($request->all(),[
         ...
         if($validator->fails()){
            ...
            $teacherRatingData= TeacherRating::where('id',$request->parent_id)->first();
            ...
            Notification::create(['sender_id'=>auth('api')->user()->id,'receiver_id'=>$selected_user_id,'type'=>4,'rating_pageId'=>$request->teacher_id,'rating_type'=>2,'is_read'=>0,'message'=>auth('api')->user()->systemNickname.' reply on your review','rating_msg_id'=>$output->id]);
            }
        //notification section
     ...
     public function addProfessorRate(Request $request)
    {
        ...
        if($validator->fails()){
           ...
           $message="Professor Reviewed successfully";
           return Helper::successMessage($message,$data);
        }
    }
    public function AddProfessorLikeDislike(Request $request)
    {
        ...
        if($validator->fails()){
            ...
            //notification section
                Helper::Notification(auth('api')->user()->id,$replyData->user_id,$notiftype,$replyData->professor_id,1,$Rnotiftype,auth('api')->user()->systemNickname.' '.$typeName.' your review',$replyId);
            //notification section
    ...
    public function addProfessorReview(Request $request )
    {
        ...
        $validator= Validator::make($request->all(),[
         ...
         if($validator->fails()){
            ...
            $professorRatingData= ProfessorRating::where('id',$request->parent_id)->first();
            ...
            if(strlen($request->comment) > 200 || strlen($request->comment) < 3){
                ...
                Notification::create(['sender_id'=>auth('api')->user()->id,'receiver_id'=>$selected_user_id,'type'=>4,'rating_pageId'=>$request->professor_id,'rating_type'=>1,'is_read'=>0,'message'=>auth('api')->user()->systemNickname.' reply on your review','rating_msg_id'=>$output->id]);
                }
            //notification section
                $professorRatingData= ProfessorRating::where(['id'=>$output->id])->with(['users','selectedUser'])->first();
                 if($request->selected_user_id){
                    $message= "Reply Insert Successfully";
                }else{
                    $message= "Review Insert Successfully";
                }
                return Helper::successMessage($message,$professorRatingData);
            }
        }
    }
    ...
    public function addUniversityRate(Request $request)
    {
        ...
        if($validator->fails()){
            ...
            $message="University Rating successfully";
            return Helper::successMessage($message,$data);
        }     
    }
    public function AddUniversityLikeDislike(Request $request)
    {
        ...
        if($validator->fails()){
            ...
            $dislikes =explode(',', $replyData[$RtypeName]);
            //notification section
                Helper::Notification(auth('api')->user()->id,$replyData->user_id,$notiftype,$replyData->university_id,3,$Rnotiftype,auth('api')->user()->systemNickname.' '.$typeName.' your review',$replyId);
    ...
    public function addUniversityReview(Request $request )
    {
        ...
        $validator= Validator::make($request->all(),[
         ...
         if($validator->fails()){
            ...
            $universityRatingData= UniversityRating::where('id',$request->parent_id)->first();
        ...
        $data=[];
            ...
            Notification::create(['sender_id'=>auth('api')->user()->id,'receiver_id'=>$selected_user_id,'type'=>4,'rating_pageId'=>$request->university_id,'rating_type'=>3,'is_read'=>0,'message'=>auth('api')->user()->systemNickname.' reply on your review','rating_msg_id'=>$output->id]);
            }
        //notification section
