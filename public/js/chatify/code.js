...
/**
 *-------------------------------------------------------------
 * Update the chat notification dropdown
 *-------------------------------------------------------------
 */

function updateChatNotifyArea(url) {
    $.ajax({
        url: url + "/updateMessageNotifications",
        method: "GET",
        data: { _token: csrfToken },
        dataType: "JSON",
        success: data => {
            $(".chatify-msg-notify ul").html(data.message);
            $(".info-dropdown-item.countmessages").text(data.messageCount);
        },
        error: error => {
            // setContactsLoading(false);
            console.error(error);
