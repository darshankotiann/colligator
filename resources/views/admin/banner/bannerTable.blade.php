...
<table id="bannerTable" class="table table-striped table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                ...
                                if($allBanners){
                                 foreach ($allBanners as $ukey => $svalue) { 
                                    ...
                                    <tr>
                                        ...
                                        <td>     <?php $teacherBanner = $svalue->image ? 'public/banner/' . $svalue->image : 'public/frontend/images/search-prof-bg1.png'; ?>
                                        <?php $mime = mime_content_type($teacherBanner); ?>
                                        @if (strstr($mime, 'image/'))
                                            <img src="{{ asset($teacherBanner) }}" id="userImage" width="50" />
                            ...
                            <?php }
                             ?>
                                <a href="{{route('admin.banner.status',base64_encode($svalue->id))}}" class="<?= $svalue->status==1 ? 'text-danger ' : 'text-success ' ?> mr-3" onclick="return confirm('Are You Sure You want to '+'<?= $svalue->status==1 ? 'Deactivate ' : 'Activate ' ?>'+'this Banner')" data-toggle="tooltip" data-placement="top" title="" oncontextmenu="return false" data-original-title="<?= $svalue->status==1 ? 'Deactivate' : 'Activate' ?>"><i class="mdi mdi-account-lock font-size-18"></i></a> 
                            </td>
                                    </tr>
                                <?php } } ?>
                            </tbody>
