...
@endif
                        ...
                        <div class="row card-title-desc">
                            <p class=" col-6 "> List Of all Comments.</p>
                          
                            
                            <div class="form-group mb-0 col-6">
                                {{ Form::open(array('action' => 'Admin\ReportSpamController@'.$ajax,'method'=>'post')) }}
                                @csrf
                                <div class="row">
                                <label class="col-3">Range Filter</label>
                                    <div class="input-daterange input-group col-6" data-provide="datepicker" data-date-format="dd M, yyyy" data-date-autoclose="true">
                                     ...
                                     <a style="float: right;" class="col-2 btn btn-danger waves-effect waves-light mr-2" href="{{route('admin.report_spam.'.$backurl)}}">Reset  </a>
                                </div>
                                {{ Form::close() }}
                            </div>
                        </div>
                        <div class="maintable">
                            @include('admin.report.allCommentTable')
                        </div>
                    </div>
                </div>
            </div> <!-- end col -->
    ...
    function showReporters($id){
        $.ajax({
            ...
            success:function(res){
                ...
                //     html+=`<a target="_blank" href="${baseurl}/admin/users/edit/${id}"> ${result}</a></br>`;
                // });
