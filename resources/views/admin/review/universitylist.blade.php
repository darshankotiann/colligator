...
@endif
                        ...
                        <div class="row card-title-desc">
                            <p class=" col-6 "> List Of all University Review.</p>
                          
                            
                            <div class="form-group mb-0 col-6">
                                {{ Form::open(array('action' => 'Admin\ReviewController@universityRangeSearch','method'=>'post','id'=>'rangeForm')) }}
                                @csrf
                                <div class="row">
                                <label class="col-3" style="padding-left:95px;">Range Filter:</label>
                                    <div class="input-daterange input-group col-6" data-provide="datepicker" data-date-format="dd M yyyy" data-date-autoclose="true">
                                     ...
                                     <a style="float: right;" class="col-2 btn btn-danger waves-effect waves-light mr-2" href="{{route('admin.universityreview')}}">Reset  </a>
                                </div>
                                {{ Form::close() }}
                            </div>
                        </div>
                        <div class="maintable">
                            @include('admin.review.universityReviewTable')
                        </div>
                    </div>
                </div>
            </div> <!-- end col -->
...
<script>
        function showReporters($id,$type='universityReview'){
        $.ajax({
            'type':'post',
