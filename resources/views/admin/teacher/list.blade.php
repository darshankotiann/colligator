...
@endif
                        ...
                        <div class="row card-title-desc">
                            ...
                            <div class="form-group mb-0 col-6">
                                {{ Form::open(array('action' => 'Admin\TeacherProfileController@RangeSearch','method'=>'post','id'=>'rangeForm')) }}
                                @csrf
                                <div class="row">
                                <label class="col-3 float-right"><p style="float:right;">Range Filter:</p></label>
                                    <div class="input-daterange input-group col-6" data-provide="datepicker" data-date-format="dd M yyyy" data-date-autoclose="true">
                                     ...
                                     <a style="float: right;" class="col-2 btn btn-danger waves-effect waves-light mr-2" href="{{route('admin.teacher.index')}}">Reset  </a>
                                </div>
                                {{ Form::close() }}
                            </div>
                            <div class="col-2">
                                <button type="button" class="btn btn-primary waves-effect waves-light w-100" data-toggle="modal" data-target="#staticBackdrop">Import Teacher</button>
                                @error('import_file')
                            ...
                            if(Auth::guard('admin')->user()->role==2){
                                ...
                                if(!empty($permissions) && $permissions['add']==1){ ?>  
                                    <a style="float: right;" class="col-2 btn btn-primary waves-effect waves-light h-100" href="{{route('admin.teacher.create')}}">Add teacher  <i class="ri-arrow-right-line align-middle ml-2"></i> </a>
                                <?php }
                            }else
                            { ?>
                                <a style="float: right;" class="col-2 btn btn-primary waves-effect waves-light h-100" href="{{route('admin.teacher.create')}}">Add teacher  <i class="ri-arrow-right-line align-middle ml-2"></i> </a>
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            ...
                                            {{Form::open(array('route'=>'admin.teacher.import','method'=>'post','enctype'=>'multipart/form-data', 'id' => 'teacherImportForm'))}}
                                        ...
                                        <div class="modal-footer">
                                            ...
                                            <button type="submit" class="btn btn-primary waves-effect waves-light" id="teacherupload">Upload</button>
                                        </div>
                                            {{Form::close()}}
                                    </div>
                                </div>
...
<script>
    $(document).ready(function() {
        $("#teacherImportForm").submit(function() {
                $("#teacherupload").attr("disabled", true);
