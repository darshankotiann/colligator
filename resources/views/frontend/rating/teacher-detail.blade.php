...
<!--                             @php $profile= $teacherData->profile ? 'teacher/'.$teacherData->profile : 'profile/colleicon.png' ;
            ...
            <div class="col-md-3 col-sm-4">
                    <div class="info-item-blk {{App\Helpers\Helper::getColor(round($schoolrange/$rowcount,2))}}">
                        <p>{{__('lang.ratings')}}</p>
                        <h3>{{number_format((float)$schoolrange/$rowcount, 1, '.', '')}}/10</h3>
                    </div>
                </div>
                <div class="col-md-3 col-sm-4">
                    <div class="info-item-blk {{App\Helpers\Helper::getColor(round($easyrange/$rowcount,2))}}">
                        <p>{{__('Easiness')}}</p>
                        <h3>{{number_format((float)$easyrange/$rowcount, 1, '.', '')}}/10</h3>
                    </div>
                </div>
                <div class="col-md-3 col-sm-4">
                    <div class="info-item-blk {{App\Helpers\Helper::getColor(round($homerange/$rowcount,2))}}">
                                        ...
                                        <div class="col-md-8 text-right">
                                            <div class="rated-items items_none_md">
                                                <ul class="teacher__ul">
                                                    ...
                                                    <li><a class="{{App\Helpers\Helper::getColor($ratingData['replyMessage']['schoolrange'])}}">{{__('lang.ratings')}}: {{$ratingData['replyMessage']['schoolrange']}}</a></li>
                                                    <li><a class="{{App\Helpers\Helper::getColor($ratingData['replyMessage']['easyrange'])}}">{{__('lang.easiness')}}: {{$ratingData['replyMessage']['easyrange']}}</a></li>
                                                    <li><a class="{{App\Helpers\Helper::getColor($ratingData['replyMessage']['homerange'])}}">{{__('lang.homework')}}: {{$ratingData['replyMessage']['homerange']}}</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
...
?>
                                                    ...
                                                    <span class="likecount {{isset(auth()->user()->id ) && in_array(auth()->user()->id,explode(',',$children['likes']) ) ? 'userLike' : ''}}">{{!empty($res[0]) ? count(explode(',',$children['likes'])) : ''  }}
                                                            ...
                                                            <li>
                                                                <a href="javascript:void(0);" onclick="replyChildUser(this,'{{base64_encode($children['userData']['id'])}}','{{base64_encode($children['id'])}}','{{($children['id'])}}')">
...
if (in_array(auth()->user()->id, explode(',', $children['report']))) {
	...
	$msg = "Are you sure You want to report/spam it";
                                                        ...
                                                        <form action="{{route('frontend.add_teacher_review')}}" method="post" class="teacherReview">
                                                            ...
                                                            <input type="hidden" name="id" class="id" value="{{base64_encode($ratingData['replyMessage']['id'])}}">
                                                            <input type="hidden" name="teacherId" class="TId" value="{{base64_encode($teacherData->id)}}">
                                                            <div class="form-group parentname child-university chieldsmall-class{{($children['id'])}}" id="childid_{{($teacherData->id)}}">
                                                            <div class="childCross"><span class="childname"></span><i class="fa fa-times crossIcon" onclick="removeReply(this)" style="right: 72px;width: auto;margin: auto;position: absolute;top: 10px; display:none"></i></div>
                                                                <div class="counterdiv">
                                        ...
                                        <ul class="rating_ul_class">
                                            ...
                                            <div class="rating-comment-form-blk">
                                                <form action="{{route('frontend.add_teacher_review')}}" method="post" class="teacherReview">
                                                    ...
                                                    <input type="hidden" name="teacherId" class="TId" value="{{base64_encode($teacherData->id)}}">
                                                    <div class="form-group parentnamee">
...
function replyUser(e,uid,id) {
    ...
    // $(".parentname textarea").css("padding-top", "6px");
    if(authUser){
