<li>
    <div class="rating-replyed-item">
        ...
        <?php
$color = '';
if ($teacherRatingData['selectedUser']) {
	$color = $teacherRatingData['selectedUser']['gender'] == 1 ? '#005BDF' : '#ED0E69';
}
?>
        <p class="description"><span style="color : {{$color}}">{{$teacherRatingData['selectedUser']['systemNickname']? '@'.$teacherRatingData['selectedUser']['systemNickname'].' : ':''}} </span> {{str_replace( $abouseWords , "****" , $teacherRatingData['message'] )}}</p>
        <hr>
        <?php
$firstId = '';
$like = '';
