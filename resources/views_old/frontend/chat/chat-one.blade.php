        ...
        $.ajax({
            ...
            $('.chatMsgBox').attr('disabled',false);
                ...
                if(response.status=='error'){
                    ...
                    var htmltext= '<div class="active-user-message-box"><span class="message">'+response.message+'</span><span class="date-time"> '+response.createdDate+'</span></div>';
                ...
                if(data.status=='error'){
                    ...
                    $.each(data.result, function(k, v) {
                        if(moment(v.created_at).format('MMMM Do YYYY') != lastDate){
                            ...
                            if(moment(v.created_at).format('MMMM Do YYYY') == moment().format('MMMM Do YYYY')){
                                htmldata+='<li class="dateLine">Today</li>';
                            }
                            else{
                                htmldata+='<li class="dateLine">'+moment(v.created_at).format('D MMM YYYY')+'</li>';
                            }
                        }
                        console.log('v========',v);
                        var uId= "{{auth()->user()->id}}";
                        var Reply= uId==v.userId ? 'reply' : 'revert';
                        htmldata+='<li class="'+Reply+'"><div class="active-user-message-box"><span class="message">'+v.message+'</span><span class="date-time">'+moment(v.created_at).format('h:mm A')+'</span></div></li>';
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(data, function(k, v) {
                ...
                htmldata+='<li class="'+classStyle+mcountStyle+'"><a href="'+hrefData+'" class="chat-box-user-list-item"><div class="chat-box-user-list-item-dtl "><div class=""><h4 class="'+gender+'" style="font-size: 16px;font-weight: 600;font-family:Aktiv-Bold;word-break: break-all;">'+userName+' </h4></div></div>';
